...
void ArduCAM::OV2640_set_agc_enable(uint8_t enable) 
{
...
void ArduCAM::OV2640_set_gain(uint8_t gain) 
{
	if(gain < 0) {
        gain = 0;
    } else if(gain > 30) {
	...
	wrSensorReg8_8(BANK_SEL, BANK_SENSOR);
	wrSensorReg8_8(GAIN, agc_gain_tbl[gain]);

	// write_reg(sensor, BANK_SENSOR, GAIN, agc_gain_tbl[gain])
}

void ArduCAM::OV2640_set_aec_enable(uint8_t enable)
{
	// write_reg_bits(sensor, BANK_SENSOR, COM8, COM8_AEC_EN, enable?1:0)
	OV2640_set_reg_bits(BANK_SENSOR, COM8, 0, COM8_AEC_EN, enable ? 1:0);
}

// Exposure is specified as percentage [0, 100]
void ArduCAM::OV2640_set_exposure(int percent)
{
	// convert to # of rows
	int exposure = (int)((1200 * percent) / 100);
    if (exposure < 0) {
        exposure = 0;
    } else if (exposure > 1200) {
        exposure = 1200;
    }

	OV2640_set_reg_bits(BANK_SENSOR, REG04, 0, 3, exposure & 0x03);
	wrSensorReg8_8(AEC, (exposure >> 2) & 0xFF);
	OV2640_set_reg_bits(BANK_SENSOR, REG45, 0, 0x3F, exposure >> 10);
}

void ArduCAM::OV2640_set_awb_enable(uint8_t enable) 
{
	// return set_reg_bits(sensor, BANK_DSP, CTRL1, 3, 1, enable?1:0);
	OV2640_set_reg_bits(BANK_DSP, CTRL1, 3, 1, enable ? 1:0);
}

void ArduCAM::OV2640_set_reg_bits(uint8_t bank, uint8_t reg, uint8_t offset, uint8_t mask, uint8_t value)
{
    int ret = 0;
    uint8_t c_value, new_value;

	wrSensorReg8_8(BANK_SEL, bank); // Bank select
	rdSensorReg8_8(reg, &c_value); // get the current value
	new_value = (c_value & ~(mask << offset)) | ((value & mask) << offset);
	wrSensorReg8_8(reg, new_value);
